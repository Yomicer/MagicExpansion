...
public class RecipeMachineCrude extends AbstractElectricRecipeMachine {
    ...
    public RecipeMachineCrude(ItemGroup itemGroup, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe, double pc1, double pc2) {
	...
	public List<ItemStack> getDisplayRecipes() {
        ...
        for (MachineRecipe recipe : recipes) {
            ...
            for (int i = 0; i < maxItems; i++) {
                ...
                // 添加输出物品（带生产时间）
                if (i < outputs.length) {
                    display.add(addLore(outputs[i], "§7成功生产概率: §a" + percent1 + "% ~ " + percent2 + "%"));
                } else {
    ...
    protected void addOutputs(BlockMenu menu, Block b, ItemStack[] outputs) {
        for (ItemStack output: outputs) {
            if (output != null) {
                if(executeCraft(percent1, percent2, b))
    ...
    public boolean executeCraft(double pc1, double pc2, Block block) {

        double min = Math.min(pc1, pc2);
        double max = Math.max(pc1, pc2);
        double pc3 = random.nextDouble(min, max + 1);
        double pc4 = random.nextDouble(1, 101);
