...
import static io.Yomicer.magicExpansion.Listener.SlimefunRegistryFinalized.ARMOR_FORGE_RECIPES;
...
public class MagicExpansionQuickArmorForge extends SimpleSlimefunItem<ItemUseHandler> implements NotPlaceable {
    ...
    private static final List<Map<String, Integer>> ARMOR_FORGE_RECIPES = new ArrayList<>();

    static {
        // 在类加载时预加载所有 SMELTERY 类型的配方
        List<SlimefunItem> smelteryItems = Slimefun.getRegistry().getAllSlimefunItems().stream()
                .filter(item -> item.getRecipeType() == RecipeType.ARMOR_FORGE)
                .toList();
        ...
        for (SlimefunItem item : smelteryItems) {
            ...
            // 将整合后的配方存入列表
            ARMOR_FORGE_RECIPES.add(recipeMap);
        }
//        System.out.println("预加载的ArmorForge配方列表：" + ARMOR_FORGE_RECIPES.size());
    }
